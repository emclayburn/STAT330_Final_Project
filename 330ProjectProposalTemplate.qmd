...
```
It's interesting to see that the more hits and runs you have, the higher your win percentage. The coorelation is very strong when ploting the points. 

```{r}
ggplot(baseball, aes(x=R - RA, y=WinPct, color = teamID)) +
  geom_point()
```
The run differential has a very strong coorelation. The more hits you have compared to your opponents, the higher your win percentage is.
...
```{r}
lm.fit = lm(WinPct~RA+ERA+E, data = baseball_simplified)
```
