...
We plan on using a baseball dataset that includes wins above average by position for each MLB team this past season. This shows each position in the field, the total for the fielders, all different kinds of pitchers, and the collective pitchers.  

```{r}
baseball <- read_csv("Teams.csv")
baseball |>
  select(yearID, teamID, Rank, W, L, DivWin, WCWin, LgWin, WSWin, H, R, RA, ERA, E, attendance) |>
  filter(yearID >= 1998)
...
Which position effects the wins above replacement the most? 
