...
$WinPct_i$ = $\beta\_0$ + $\beta\_1$ \* $ERA_i$ + $\beta\_2$ \* $E_i$ + $\epsilon$
...
For Beta_0, when both ERA and Errors are zero, the WinPct is .9179
For Beta_1, holding all else constant, for every one increase in ERA, the WinPct goes down by an average of .0874
For Beta_2, holding all else constant, for every one increase in Errors, the WinPct goes down by an average of .0005

The p-values for the individual testing of the coefficients based on H_0 = 0 are really significant with our F-statistic p-val being extremely less than 0.05
...
summary(lm.fit)
```
Using the avPlot, we can see that the data is linear, meaning our first assumption is met. We know that independence is also met because the way that the data is gathers means the data is independent
```{r}
#N
plot(lm.fit, which = 2)

```
Here we can see that the errors are very normally distributed and we can accept that assumption is met.
```{r}
#E
...
```
When looking at the fitted vs residuals plot, the values are scattered and in a cloud, meaning that there is equal variance.
...
vif(lm.fit)
```
Here we can see that the model coefficient vifs are below 5 and are thus the model does not experience extreme multicollinearity.

```{r}
#influential points
plot(lm.fit, which = 5)
```
Here we can see that there are really no extreme influential points above the 0.5 threshold and won't have a drastic effect on our analysis.
