...
For Beta_0, when both ERA and Errors are zero, the WinPct is ---
For Beta_1, holding all else constant, for every one increase in ERA, the WinPct goes (up/down) by an average of ---
For Beta_2, holding all else constant, for every one increase in Errors, the WinPct goes (up/down) by an average of ---

The p-values for the individual testing of the coeffecients based on H_0 = 0 are really significant with our F-statistic p-val being extremely less than 0.05
...
vif(lm.fit)
```
Here we can see that the model coeffecient vifs are below 5 and are thus the model does not expereince extreme multicollinearity.
```{r}
#N
plot(lm.fit, which = 2)
...
plot(lm.fit, which = 5)
```
Here we can see that there are really no extreme influential points above the 0.5 threshold and won't have a drastic effect on our anlysis.
